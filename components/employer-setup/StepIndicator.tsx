...
const StepIndicator: React.FC<StepProps> = ({ title, description, icon, isCompleted, isActive }) => (
  <div className={`flex items-center space-x-4 p-4 rounded-lg transition-all duration-200 ${
    isActive ? 'bg-blue-50 border-2 border-blue-200' : 'bg-white border-2 border-gray-200'
  }`}>
    ...
    <div className="flex-grow">
      <h3 className={`font-semibold ${isActive ? 'text-blue-900' : 'text-gray-700'}`}>{title}</h3>
