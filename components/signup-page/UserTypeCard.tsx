...
const UserTypeCard: React.FC<TypeCardProps> = ({ type, title, description, icon, benefits, isSelected, onSelect }) => (
  <div
    onClick={onSelect}
    ...
    <ul className="space-y-2">
      {benefits.map((benefit, index) => (
        <li key={index} className="flex items-center space-x-2 text-sm text-gray-600">
          <CheckCircle size={16} className={isSelected ? 'text-blue-500' : 'text-gray-400'} />
          <span>{benefit}</span>
        </li>
      ))}
    </ul>
